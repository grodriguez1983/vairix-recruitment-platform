...
**Última sesión**: 2026-05-22 — **Bloque 30: ADR-036 — soft union pre-filter gate (descartar candidatos con cero overlap antes del ranker)**. Owner observó en `/matching/new` corriendo un JD con dos requirements ambos `must_have=false` (React + Node) que el pipeline FE-driven procesaba el pool entero (~8 692 candidatos) — ~10 min para llegar a 4 300/8 692. Pregunta del owner: "qué sentido tiene traer candidatos a analizar que no tienen ninguna de las tecnologías pedidas?" Diagnóstico: el pre-filter (RPC `match_pre_filter`, ADR-033) usa **must_have_groups** como única compuerta; sin must-have resueltos, devuelve `included = todos los candidatos visibles`. Las soft skills son señales de scoring pero **nunca filtran**. Para un JD soft-only, todo candidato sin overlap entraba al ranker condenado a `total_score 0` — desperdicio estructural del round-trip de `match_load_aggregates`. **Decisión** (ADR-036): segundo gate aditivo _soft union gate_ — sea `any_of_skill_ids` = unión de `requirement.skill_id` sobre todo requirement resuelto (must ∪ soft); candidato incluido sii cubre todos los must-have groups activos (gate 1, inalterado) Y, si la unión es no vacía, tiene ≥1 fila en `experience_skills` con `skill_id ∈ any_of_skill_ids`. JD sin nada resuelto ⇒ ambos gates inactivos ⇒ comportamiento previo. JD soft-only ⇒ gate 2 filtra cero-overlap. Los excluidos por gate 2 **no entran al excluded pool** — el rescue FTS de ADR-016 busca must-have en `parsed_text`, no candidatos sin overlap soft. **Por qué unión y no intersección**: una intersección rompería la semántica de "soft" (pedir Tailwind+Sass como soft no debe excluir a un dev con solo Tailwind). **Por qué no en el ranker**: la decisión "este candidato no tiene nada de lo pedido" es resolvable en SQL antes de cargar aggregates — ~10-100× menos costo que merge de variantes + idiomas. **Pure impl** (`src/lib/matching/pre-filter.ts`): nuevo `collectAnyOfSkillIds()` exportado; `preFilterByMustHave` re-estructurada para evaluar ambos gates en un solo round-trip de coverage (la unión incluye must-have ⊂ anyOf, así que la query del must-have sirve para los dos). **Migration `20260522000001_match_pre_filter_any_of.sql`**: drop de la firma `(jsonb, uuid)` + create `match_pre_filter(jsonb, uuid, uuid[] default null)`; el cuerpo aplica el gate 2 dentro del CTE `visible_candidates` (filtra `exists(experience_skills ∩ any_of_skill_ids_in)` antes del cálculo de coverage); `default null` ⇒ clientes legacy llamando con dos args resuelven la nueva firma sin cambio. **3VL gotcha**: `array_length('{}'::uuid[], 1)` devuelve **NULL**, no 0 — la primera versión del guard usaba `... and array_length(...) > 0` que se evaluaba a NULL ⇒ falsy en early-return pero downstream el `not any_of_active or exists(...)` con NULL daba NULL ⇒ todos los candidatos filtrados. Caught por el test integration "empty array disables gate 2"; fix con `coalesce(array_length(...), 0) > 0`. **Adapter** (`db-deps.ts:preFilter`): deriva `anyOf` via `collectAnyOfSkillIds()` y lo pasa al RPC (null si vacío). Log line incluye `any_of=N` para diagnóstico. **Tests**: 4 unit nuevos en `pre-filter.test.ts` (soft-only union excluye zero overlap, must+soft union ⊇ must, unresolved-only desactiva ambos gates, contract de `collectAnyOfSkillIds`) + 4 integration en `match-pre-filter-rpc.test.ts` (soft-only RPC behavior, must+soft con cand-E filtrado por gate 2 antes de evaluación de missing-groups, null vs omitido equivalentes, array vacío desactiva gate 2). 17/17 verde en pre-filter.test, 18/18 en db-deps.test, 11/11 en match-pre-filter-rpc.test, 227/227 en todo `src/lib/matching` + `tests/integration/matching`. Typecheck + lint limpios. **Bug previo**: test existente "multiple resolved must-have" asumía que la query de coverage recibía solo IDs must-have; bajo ADR-036 recibe la unión (incluye SQL_ID soft) — assertion actualizada con explicación inline. **3 commits** (`f026794` docs, `1feedfa` feat impl+migration, `946db4e` test integration). **Migración aplicada a remoto** vía `supabase db push --linked` autorizado por owner — la próxima corrida en prod ya filtra por unión. **Pendiente**: (a) owner valida en prod con el mismo JD del incidente — esperado: `included` ya no es ~8 692 sino el subset con ≥1 overlap (probablemente cientos); (b) ADR-035 sigue pendiente de validación end-to-end del flow editable (Bloque 29); (c) considerar consolidar `any_of_skill_ids` en un `gates_in jsonb` si en el futuro emergen más gates aditivos (notado en ADR §Costos). **Bloque 29 (previo, 2026-05-22)**: ADR-035 — recruiter-editable decomposition override.
**Última sesión previa**: 2026-05-22 — **Bloque 29: ADR-035 — recruiter-editable decomposition override por match run**. Owner pidió poder editar el resultado del decompose en `/matching/new` antes de correr el match: ajustar `min_years` por requirement, togglear `must_have`, eliminar requirements del set. Colisión inmediata con ADR-014 §3 (cache de `job_queries` compartido por `content_hash`): PATCH a `resolved_json` poluye cross-user. Decisión consensuada — **Opción A**: el override viaja en el body de `POST /api/matching/run/start` y se persiste como snapshot inmutable en `match_runs.effective_resolved_json` (nueva columna jsonb, freezada por el state-machine trigger desde insert). Regla de subset defensiva en el service (`validateOverrideIsSubset` en `src/lib/matching/start-match-run.ts`): identidad del requirement (`skill_id`+`alternative_group_id`+`category`+`skill_raw`+`evidence_snippet`) la fija el LLM y no se puede mutar; sólo `must_have`/`min_years`/`max_years` son editables; eliminar requirements OK; agregar nuevos NO; `seniority`/`languages`/`notes`/`role_essentials` deben coincidir con el original (out of scope para edit este pase). **Migration `20260522000000_match_runs_effective_resolved_json.sql`**: alter table aditivo + reemplazo del cuerpo de `enforce_match_runs_state_machine()` para freezar la columna nueva siempre (identity, no sólo post-close). Backwards-compat: runs históricos quedan con `effective_resolved_json = null`; `buildProcessMatchChunkDeps.loadRunForChunk` cae a `job_queries.resolved_json` como fallback. **TDD estricto**: RED commit conceptual con 8 tests adversariales nuevos en `src/lib/matching/start-match-run.test.ts` (no override → persists loaded.resolved before preFilter; with override → preFilter receives override; persist receives override; rejects new skill_id; rejects swapped skill_id; rejects changed seniority; accepts delete; accepts must_have+min_years edits). 6 fallaban contra el impl original; GREEN con `validateOverrideIsSubset` (pure) + nueva dep `persistEffectiveResolved` + `effective = override ?? loaded.resolved` aplicado a preFilter y al snapshot. 19/19 verde en `start-match-run.test.ts`, 201/201 en `src/lib/matching`, 874/874 en `test:unit`. Typecheck + lint limpios. **Route**: `startMatchRequestSchema` extendido con `resolved_override?: resolvedDecompositionSchema` (zod local — schema canónico de `ResolvedDecomposition` no existía, se declaró en el route con shape derivada de tipos de `decomposition/types.ts`). Mapeo `400 invalid_override` con regex sobre el mensaje del service (mismo patrón que `job_query_not_found → 404`). **FE** (`src/app/(app)/matching/new/new-match-form.tsx`): state `editedResolved: ResolvedDecomposition | null` lifteado a `NewMatchForm`, clonado via `JSON.parse(JSON.stringify(...))` para no mutar la respuesta del server. `DecompositionPanel` deja de ser read-only — extrae `RequirementRow` con checkbox `must_have`, input `number` min/max=0/50 (vacío → `null`) y botón `×` para remover; badge `· edited` y botón `reset` cuando hay dirty. `onRun` calcula `hasEdits` por deep-stringify y sólo manda `resolved_override` cuando difiere (ahorra bytes y deja audit clean cuando el recruiter no tocó nada). `effective_resolved_json` también lo lee `/process-chunk` para que el ranker corra contra el set efectivo (no contra el LLM original). **Types**: `pnpm supabase gen types typescript --local` regeneró `src/types/database.ts` con la columna nueva. Tuve que strip-ear 4 líneas de stderr del CLI (`WARN: no seed files...` + `ExperimentalWarning`) que se filtraban al stdout y rompían el parse. **Pendiente**: (a) owner valida el flow real en local — editar requirements, correr, verificar en `/matching/runs/:id` que `effective_resolved_json` está sealed; (b) tests de route HTTP para el path `400 invalid_override` (no agregados — el service cubre la lógica del subset); (c) UI eventual para `seniority`/`languages` editables si emerge la necesidad (no en scope de este ADR). **Bloque 28 (previo, 2026-05-21)**: prompt v7 — rule 12 minimal/single-token JDs.
**Última sesión previa**: 2026-05-21 — **Bloque 28: prompt v7 — rule 12 minimal/single-token JDs (fix "busco programador react → 0 requirements")**. Owner reportó screenshot de `/matching/new` con JD `busco programador react` mostrando `Decomposition · CACHE HIT · REQUIREMENTS (0)` y `RUN MATCH` habilitado vacío. Root cause: el prompt v6 con Rule 6 (`Do not invent requirements... we prefer false negatives to fabricated requirements`) le indicaba a `gpt-4o-mini` que ante una JD ultra-informal sin sección de requisitos se quede en silencio, aún cuando el nombre de la tecnología aparece literalmente como substring (cumpliría Rule 3 / evidence_snippet). El resultado quedó cacheado en `job_queries.decomposed_json` keyed por `content_hash = SHA256(normalized || NUL || model || NUL || promptVersion)` (`src/lib/rag/decomposition/hash.ts:5`) → toda llamada posterior con el mismo texto sigue dando 0. Owner eligió **Option 1** (prompt fix) sobre Option 2 (UX guard min-chars) y Option 3 (FE warning post-decompose). **Decisión** — agregar Rule 12 carve-out al prompt: cuando el input es un fragmento corto (≤~15 palabras) que nombra una o más tecnologías como keyword del rol (e.g. "busco programador react", "dev rails", "react native dev", "node engineer"), emitir una requirement por tecnología con `must_have: false`, `min_years/max_years: null`, `evidence_snippet = texto completo`, `skill_raw = nombre canonical` (rule 4), `alternative_group_id: null`, `role_essentials: []`. Rule 6 sigue aplicando para JDs largas. Bump `DECOMPOSITION_PROMPT_V1` `2026-04-v6 → 2026-05-v7` (`src/lib/rag/decomposition/prompts/decompose-v1.ts:14`) — como `promptVersion` entra en el hash, la invalidación del cache es automática: las filas viejas en `job_queries` quedan inertes (nunca más las consulta), la próxima llamada con el mismo texto re-dispara una LLM call con el prompt nuevo. No requiere migración ni purge manual. **Tests**: pin de versión flipado en `src/lib/rag/decomposition/prompts/decompose-v1.test.ts` + nuevo `describe('minimal-JD rule 12 (prompt v7)')` con 3 regression assertions: (a) prompt menciona "minimal/short informal/single-token" + pin literal "busco programador react"; (b) prompt especifica `must_have: false` y `min_years null` para el carve-out; (c) prompt scope explícito al fragmento corto (`rule 6 still applies | longer JDs`). 20/20 tests verde en `prompts/decompose-v1.test.ts`, 116/116 en todo `src/lib/rag/decomposition`. Typecheck + lint limpios. **Pendiente**: (a) owner verifica el flow real en local/prod tipeando "busco programador react" — esperado: cache miss → fresh LLM call → 1 requirement React `must_have=false`; (b) decidir si esto amerita un ADR-035 micro o queda como amendment de ADR-014 §3 — propongo amendment porque no cambia el contrato (sigue siendo "no inventar"; sólo precisa cuándo el modelo NO está inventando); (c) considerar Option 2/3 como defense-in-depth si aparecen otros casos donde el modelo sigue siendo conservador (JD copy-pasteada con tipos raros). **Bloque 27 (previo, 2026-05-21)**: ADR-034 — FE-driven chunked matching pipeline.
**Última sesión previa**: 2026-05-21 — **Bloque 27: ADR-034 — FE-driven chunked matching pipeline (kill H12 estructuralmente)**. Owner confirmó que ADR-031 (parallel chunked-IN, Bloque 26) seguía tocando techo a 513 candidatos elegibles: `loadCandidates: FAILED after 60428ms code=57014 message=canceling statement due to statement timeout`. Pre-filter ya tomaba 11s; `loadCandidates` quemaba el statement_timeout=60s del worker pool por su cuenta. La estrategia "subir paralelismo" no escala — el budget hard de Heroku H12 + statement_timeout sigue ahí. **Decisión** (ADR-034): partir el endpoint sync en 3 endpoints stateless driven por el FE: `POST /api/matching/run/start` (abre run, corre preFilter, stampa `expected_count`, devuelve `{run_id, included[], excluded[], total}`); `POST /api/matching/run/process-chunk` (workhorse, N llamadas con slices de `included`, ranking + insert chunk-local, `bumpProgress`); `POST /api/matching/run/finalize` (cierra run, lee top-N por `total_score DESC`, dispara rescue, completa). **Migration 20260521000004** (`ae90e3f`): agrega `expected_count`, `processed_count`, `last_progress_at` a `match_runs` + `idx_match_results_run_score (match_run_id, total_score DESC)` para el read final-top-N. **Option 1**: `match_results.rank` queda chunk-local, NO hay re-rank global — el insert-only trigger de ADR-015 §5 bloquea UPDATE en rank de todas formas, y el índice ordena por score directamente. **TDD estricto** por servicio: RED→GREEN→wire para `startMatchRun` (`9c87950`/`757486e`/`c1a906d`), `processMatchChunk` (`b33fabe`/`330e59f`/`b996f5d`), `finalizeMatchRun` (`6ee2854`/`4083d67`/`dac9b57`). Sub-deps inyectadas para test puro: `loadJobQuery`, `createMatchRun`, `preFilter`, `setExpectedCount`, `failMatchRun`, `loadRunForChunk`, `loadCandidates`, `rank`, `insertMatchResults`, `bumpProgress`, `loadRunForFinalize`, `loadTopResults`, `loadFailedResults`, `rescueFailedCandidates?`, `completeMatchRun`. Adapter builders en `db-deps.ts` (`buildStartMatchRunDeps`, `buildProcessMatchChunkDeps`, `buildFinalizeMatchRunDeps`) reúsan los helpers del legacy `runMatchJob` donde aplica; `collectFailedCandidates` promovido a exported en `run-match-job.ts` (única fuente del derivado `breakdown_json` → missing-skill-ids para rescue). **Error model**: `/start` llama `failMatchRun` si rompe post-createMatchRun. `/process-chunk` y `/finalize` rethrow sin tocar el run — el FE owns lifecycle, retries con la misma `run_id`. Rescue es orthogonal (ADR-016 §1), errores swallowed. **FE migration** (`112f08e`, `src/app/(app)/matching/new/new-match-form.tsx`): reemplaza el único `POST /api/matching/run` por loop start→process-chunk × ceil(total/CHUNK_SIZE)→finalize. `CHUNK_SIZE=100` conservador (loadCandidates + rank + insert para 100 cabe holgado bajo H12 — raise solo con medición). UI de progreso `pre-filtering… → 123/513 (24%) → finalizing…` reemplaza el "running…" opaco. Helper `postJson<T>` uniforma error surface. **Suite local**: 71/71 tests verdes en la trinca de servicios nuevos + integration de `runMatchJob` e2e (`db-deps-pagination`). **Deploy** (v9 en Heroku, `https://recruitment-platform-76e1cd2e6ca0.herokuapp.com/`): migration 004 push-eada al remoto vía `supabase db push` (autorización explícita del owner). `git push origin main && git push heroku main` autorizado por el owner vía `!` prefix tras hook bloqueo (`CLAUDE_ALLOW_DESTRUCTIVE=1` inline no funciona — el hook se ejecuta antes que el comando, env var no llega). Build ok, todas las rutas presentes (`/api/matching/run/{start,process-chunk,finalize}`). **PostgREST schema cache gotcha**: tras `supabase db push` PostgREST cachea el schema viejo; en remoto refresca automáticamente al detectar DDL, pero en local hay que `NOTIFY pgrst, 'reload schema'` o `supabase stop && start`. **Pendiente**: (a) owner confirma loop completo en prod contra el JD del incidente — esperado `0/513 → 100/513 → ... → 513/513` sin H12; (b) `/api/matching/run` legacy queda como dead code; decisión de borrarlo en próximo bloque (test suite todavía lo cubre); (c) Plan B server-side pre-filter RPC (de Bloque 26) sigue pendiente para cuando el pool llegue a 15k+. **Bloque 26 (previo, 2026-05-21)**: ADR-031 — parallel chunked-IN fan-out (compraba runway pero no escalaba indefinidamente; ADR-034 es el fix estructural).
**Última sesión previa**: 2026-05-21 — **Bloque 26: ADR-031 — parallel chunked-IN fan-out en matching pipeline (fix H12 timeout)**. Owner reportó `curl POST /api/matching/run` devolviendo 503 H12 Request timeout en Heroku. Diagnóstico vía `heroku logs --tail`: el endpoint pasó de 23-27s a consistentemente 30000ms→503 después de que el drain wrapper v2 (PID 8354, sigue corriendo en background) drenara ~5631 `candidate_extractions` rows (vs ~989 ayer). Pool de candidatos elegibles para matching creció ~6×. **Root cause**: en `src/lib/matching/db-deps.ts`, los tres helpers chunked-IN (`fetchCandidateMustHaveCoverage`, `loadExperiences`, `loadLanguages`) iteraban chunks de 200 ids con un for-loop manual **secuencial**. A 5631 candidates son ~29 chunks por helper × 3 helpers × ~3 page-RTT promedio × ~45ms dyno→Supabase RTT ≈ 12s solo de network sequencial, sumado al query time excede los 30s de hard cap de Heroku. **Decisión** (ADR-031): bounded-parallel dispatch en `runChunked` (`src/lib/shared/chunked-in.ts`) con `concurrency=5` default conservador contra el ceiling ~15 conn del Supavisor tenant pool. Worker-pool de tamaño `min(concurrency, chunks.length)` pulea chunks de contador compartido, escribe a slots pre-sizeados indexados por chunk-issue para preservar output order incluso con completion order distinto. Failure semantics match secuencial: primer error captura `firstError`, workers restantes salen cooperativamente, runner rethrowea, sin partial results. **Compatibilidad**: `concurrency: 1` (default) preserva el loop secuencial original (no Promise overhead) — embeddings workers (caller existente sin la opción) no se tocan. **TDD estricto**: RED `5fdb2e6` con 8 tests nuevos en `chunked-in.test.ts` (default sequential regression guard, 3-en-paralelo con 6 chunks, 3-budget con 2 chunks ceiling, preserves-chunk-issue-order-with-staggered-completion, rejects zero/negative/non-integer concurrency, on-failure-surfaces-error). 7 de los 8 pasaron contra el impl secuencial por trivialidad geométrica; el clave (in-flight count = 3) reventó como esperado — único RED de verdad. GREEN `4001e86` con el worker pool. Wire-up `406ed27` en los tres call sites de `db-deps.ts` con const local `CHUNK_CONCURRENCY = 5`. **Suite local post-fix**: 162/162 tests verde en `src/lib/matching` + `src/lib/shared` (incluye los 20 de `chunked-in.test.ts`). Embeddings workers (`profile-worker`, `evaluation-worker`, `worker-runtime`, `cv-worker`, `notes-worker`) — los 5 callers existentes de `runChunked` sin opción — 45/45 tests verde post-change. **Hook bypass** (commits GREEN y wire-up con `--no-verify`, autorización explícita del owner): 3 RAG integration tests pre-existentes fallan en `main` previo a este cambio (`tests/integration/rag/{hybrid-search,semantic-search}.test.ts` — empties on assert, sintoma típico de embeddings sin sembrar en supabase-test). Verified via `git stash` en RED commit. No relacionado con este fix; queda como follow-up (mismo drift documentado en Bloque 22 §Drift). **Pendiente**: (a) push a Heroku + medir p99 contra el mismo `job_query_id` que dio 503 (pass criterion: <25s); (b) Plan B follow-up — pre-filter via RPC server-side colapsando `fetchAllCandidateIds + fetchCandidateMustHaveCoverage + loadCandidates` en un SELECT que solo devuelve elegibles. Es el fix estructural correcto cuando el pool llegue a 15k+; este Plan A compra runway pero no escala indefinidamente. (c) Monitorear `pg_stat_activity` próximas 48h: pico esperado 10-12 conns durante run, sin alcanzar 15. **Bloque 25 (previo, 2026-05-18)**: ADR-028 addendum — date-window backfill + `--seal-cursor`.
**Última sesión previa**: 2026-05-18 — **Bloque 25: ADR-028 addendum — date-window backfill + `--seal-cursor`**. Owner pidió "traer candidatos de a poco, batches de 50/100/1000, detectar actualizados en TT y nuevos sin romper lo anterior". El modo original de `sync:backfill` (reset cursor + re-fetch desde page 1) impide eso porque al cerrar siempre re-fija el watermark en `T_now` — no hay forma de pedir un slice histórico sin pisar el incremental. **Decisión** (Option A, dos modos adicionales sin romper el default): (1) `--from=ISO --to=ISO` inyecta `filter[updated-at][from|to]` + `sort=updated-at` como `requestParamsOverride` con `cursorPolicy: 'preserve'` — el cursor queda EXACTAMENTE como estaba antes del run; (2) `--seal-cursor` pin a `now()` sin llamar TT (`UPDATE sync_state SET last_cursor=$now, last_synced_at=$now`). Receta operativa: chunkear historial por ventanas → `--seal-cursor` → `sync:incremental` arranca delta puro desde el seal. **Bug colateral resuelto previamente en la sesión** (commit `533209f`, antes del compaction): `/v1/uploads` rechaza todos los `filter[*]` con código 102; switcheado a `sort=-updated-at` + `shouldStop(resource, cursor)` client-side early-stop (patrón C′). **Cambios técnicos**: `EntitySyncer` gana `shouldStop?` opcional; `SyncerDeps` gana `requestParamsOverride?` (merge caller-wins sobre `buildInitialRequest().params`) y `cursorPolicy?: 'advance' | 'preserve'`; `ReleaseOutcome.success.lastSyncedAt` ensanchado a `string | null` para permitir preserve + entidad fresca; nuevo helper `sealCursor(db, entity, atIso)` en `lib/sync/cli.ts`; `parseBackfillArgs` valida `--from`/`--to` pareja, rechaza `--seal-cursor` con `--from` o con `--entity=all`, valida ISO-8601 + `from < to`; `backfill.ts` rutea entre 3 modos (sealCursor short-circuit, dateWindow path, default reset). **TDD estricto**: RED `e0a6ab2` con tests para shouldStop semantics, requestParamsOverride merge, cursorPolicy='preserve', parseBackfillArgs casos nuevos, sealCursor writes both fields + propaga error de DB. GREEN `bac028f` con la implementación completa. Pre-commit verde (lint, typecheck, 67 changed-scope tests). **Suite total**: 68 unit + 39 integration sync verde. **ADR-028 addendum** documenta los dos modos, la receta operativa por bloques (2024H1, 2024H2, 2025H1, 2025H2, 2026 → seal → incremental), alternativas rechazadas (`--page=N` por inestabilidad de paginación TT, `--since=ISO` sin `--to` por equivalencia a `--from=ISO --to=$now` chunkeado, mezclar reset+window por contradicción semántica). **Pendiente operativo**: ejecutar las ventanas reales contra producción (candidates → applications → notes → evaluations → files vía shouldStop) → `--seal-cursor` por entidad → `sync:incremental` baseline.
**Última sesión previa**: 2026-05-18 — **Bloque 24: bootstrap Supabase remoto (`tqyvvtmaouhmfsknfvxq`) — 60 migraciones aplicadas + ADR-009 caso confirmado**. Owner pidió "empezar a usar Supabase remoto y cargar datos en una base remota" después de mergear PR #1 (Bloque 23). El proyecto remoto fue creado el 2026-04-17 (West US Oregon) y tenía sólo migraciones 001/002 aplicadas — desincronizado con el local que llevaba 60. **Fase 1 — `supabase db push --linked --include-all`**: primer intento falló en la segunda migración (`20260417205152_app_users.sql`) con `ERROR: function uuid_generate_v4() does not exist (SQLSTATE 42883)`. Root cause: la migración 001 (`20260417201204_extensions_and_helpers.sql`) que se aplicó al remoto el 2026-04-17 era **anterior** a ADR-009 (creado al día siguiente, 2026-04-18) y por lo tanto no incluía la línea `alter database postgres set search_path = public, extensions;`. Cuando se aplicó la nueva versión del archivo en local, todo OK, pero como el `migration_id` ya estaba en `supabase_migrations.schema_migrations` del remoto, la versión nueva del archivo **no re-ejecuta**: las migraciones son immutable por id. Verificado con `select unnest(setconfig) from pg_db_role_setting s join pg_database d on d.oid = s.setdatabase where d.datname = 'postgres'` — el remoto sólo tenía `app.settings.jwt_exp`, faltaba `search_path`. **Workaround una-sola-vez** (documentado ya en ADR-009 §Notas de implementación): `supabase db query --linked "alter database postgres set search_path = public, extensions;"` directo contra el remoto. Verificación post-fix mostró ambas líneas (`jwt_exp` y `search_path=public, extensions`) en `pg_db_role_setting`. Retry de `db push` aplicó las 60 migraciones en orden sin más fricción. **Tipos regenerados** (`supabase gen types typescript --linked 2>/dev/null > src/types/database.ts`): primer intento corrupto con stderr leak ("Initialising login role..." en línea 1 del .ts), resuelto con `2>/dev/null`. Diff de 389 líneas: `__InternalSupabase: { PostgrestVersion: "14.5" }` metadata block agregado, `description_tsv: unknown | null` → `description_tsv: unknown` (generated NOT NULL), filtrado de overloads de extensión pg*trgm/vector. Commit `cd777c6`. **Fase 2 — env config**: `.env.production.local` (gitignored vía pattern `.env.*.local`) con `NEXT\*PUBLIC_SUPABASE_URL=https://tqyvvtmaouhmfsknfvxq.supabase.co`, `NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY=sb_publishable_utJJAkPUMXUo3-XHNJRJuA_VXaCHGdA` (nueva nomenclatura post ADR-003 §7: `sb_publishable\**`reemplaza`anon`, `sb*secret*\_`reemplaza`service*role`), `SUPABASE_PROJECT_REF=tqyvvtmaouhmfsknfvxq`, `TEAMTAILOR_API_TOKEN`/`OPENAI_API_KEY`heredados de`.env.local`(un solo tenant, una sola key de OpenAI).`SUPABASE_SECRET_KEY` queda como placeholder (`<paste-sb_secret*…-here>`) — el CLI devuelve el secret enmascarado (`sb*secret_0BfSZ··························`) y la dashboard sólo muestra el valor una vez al crear la key; o el owner lo copió en su momento o hay que rotarla. **ADR-009 addendum**: agregado el CLI variant `supabase db query --linked "..."`como alternativa al`psql`del párrafo original, query de verificación con`pg_db_role_setting`, y sección "Caso confirmado (Bloque 24, 2026-05-18)" documentando este incidente concreto — decisión de **no crear ADR-031**: §Notas ya cubría exactamente el escenario "migración 001 corrió antes del ADR", sólo faltaba el dato operativo del incidente real. **Gates**: typecheck + lint limpios, suite 1020/1023 (3 drift preexistente en `tests/integration/rag/{semantic,hybrid}-search.test.ts`, anotado en Bloque 22 §Drift, no relacionado). **Bloqueador para Fase 3 (auth users) y Fase 4 (data load)**: necesitamos que el owner pegue el `sb_secret*…`en`.env.production.local`(sin él,`SUPABASE_SECRET_KEY`queda vacío y los scripts de sync/extract/embed contra el remoto fallan al inicializar el client server-side). Si la key se perdió → rotar desde https://supabase.com/dashboard/project/tqyvvtmaouhmfsknfvxq/settings/api-keys. **Próximos pasos cuando el secret esté en .env.production.local**: (a) crear los 5-15 users VAIRIX en auth.users del remoto (vía dashboard o`auth.admin.createUser`); (b) Fase 4 data load — empezar con `SYNC_MAX_RECORDS=5 pnpm sync:incremental stages`(per memoria del proyecto: validar mapeo con muestras chicas antes de full sync), escalar entidad por entidad en orden canónico ADR-028 (stages → users → jobs → custom-fields → candidates → applications → notes → evaluations → files), luego`parse:cvs → extract:cvs → embed:all`. Aplica el mismo workflow que dev pero contra producción — operación Tier 2 que requiere validación en cada salto. Nunca correr `sync:full`o`sync:backfill --entity=all`directo en remoto sin gate humano.
**Última sesión previa**: 2026-05-18 — **Bloque 23: ADR-030 — recalibración`HALF_LIFE_YEARS`4 → 2**. Owner reportó caso de dominio en JD`Senior full stack laravel/php + react + redux`: Emiliano Mateu salía rank #1 score 61.3 **passed** con PHP `ratio=1.00`pese a`last_used=7y ago`("dice que no toca php ni laravel hace 7 años pero le da un ratio de 1 en lo que es php"). Aritmética con`h=4`: PHP raw≈10y → effective=10×0.5^(7/4)=2.97 → ratio=min(2.97/3,1)=1.00 contra el baseline senior=3 (ADR-022). El sistema actuaba como diseñado; el bug era **calibración**, no implementación — h=4 deja el effective todavía sobre el piso senior aún a 7 años de stale. **ADR-030** (`413b063`): one-line drop de `HALF_LIFE_YEARS = 4 → 2`en`src/lib/matching/recency-decay.ts:34`, supersedes parcialmente ADR-026 §Decisión. Resto de ADR-026 (formula, `asOf`determinístico, exclusión del gate, side_project en`lastUsed`, exposición en `breakdown_json`) preserved íntegro. Sanity checks documentados en la ADR: senior activo unchanged (factor=1 ongoing), senior con 1y sabático ratio sigue=1.0 (effective sobre baseline), junior con 1y/3y-stale cae a 0.12 (drop justo). **TDD estricto**: RED `3e7f9fb`con`TDD_RED=1`— recalibrados 6 tests numéricos:`test_decay_factor_half_at_one_half_life`(comentario),`test_decay_penalizes_15yr_old_java` (`effective < 0.05`, `factor < 0.01`), `test_decay_uses_asOf_not_wallclock`(r2014 factor=0.25 vs era 0.5, r2026 < 0.01),`test_decay_uses_latest_end_date_per_skill`(factor 0.09–0.13, effective 0.65–0.9),`test_decay_factor_at_exact_half_life`renombrado "4y → 0.5" pasa a "2y → 0.5" con fechas 2020–2024,`test_adr026_breakdown_exposes_decay_components`(decay_factor 0.2–0.25 para 4.32y), y`test_adr026_stale_skill_still_passes_role_essentials_gate`recalibrado: assertion`total_score > 0`reemplazada por`raw_years > 0 && decay_factor > 0`— bajo h=2 la contribución de 15y-stale (~0.006 × peso) no supera el penalty de seniority below (-5),`max(0, …)`colapsa a 0; eso no es el gate silenciando al candidato (el gate pasó) sino el score honestamente derritiéndose. GREEN`b0e8f4a` flip one-line. Suite matching **131/131 verde** post-flip (`pnpm vitest run src/lib/matching`). Typecheck + lint limpios. **Tabla del efecto sobre Emiliano** (asumiendo mismos raw): PHP 10y/7y stale `1.00 → 0.29`, Laravel 2.7y/7y stale `0.27 → 0.08`, React 10.5y/<1mo unchanged `1.00 → 1.00`. **Pendiente manual**: re-correr el JD del incidente contra el corpus dev y verificar (a) Emiliano cae del rank #1, (b) candidatos con PHP/Laravel activos suben relativo, (c) React activo intacto. No requiere schema change ni migración — `match_runs`históricos persisten como snapshots de su calibración.
**Última sesión previa**: 2026-05-18 — **Bloque 22:`extract:cvs`log line incluye`languages=`**. Cierra el item (e) del tech debt anotado en Bloque 21. El counter `stats.languagesInserted` ya existía en el worker (`src/lib/cv/extraction/worker.ts:114`, testeado en `worker.test.ts:234,314,330,366`) pero la línea de log final del CLI no lo emitía — operativo seguía a ciegas sobre cuántas `candidate_languages`se persistían por run. Cambio one-liner en`src/scripts/extract-cvs.ts:274`: agregado `languages=${stats.languagesInserted}` entre `skills=` y `derivationErrored=`. Sin TDD (CLAUDE.md §🧪 reserva TDD para lógica no trivial; observabilidad de un counter ya testeado no califica). Typecheck + lint limpios. Commit `e7b6adf` pusheado a `origin/main`. **Drift detectado al cerrar la sesión** (preexistente, no relacionado con este cambio): `pnpm test` reporta 1020/1023 pasando, 3 falla en `tests/integration/rag/{semantic-search,hybrid-search}.test.ts` — los asserts esperan hits semánticos contra candidatos seedeados pero el ranker devuelve listas vacías, síntoma típico de embeddings sin sembrar en test DB. Verificado en stash limpio: las fallas no dependen del diff de Bloque 22. Item nuevo en §"⚠️ Drift detectado entre docs / tests".
**Última sesión previa**: 2026-04-27 — **Bloque 21: ADR-029 — re-extract CV cuando `parsed_text` cambia + cleanup de siblings**. Owner preguntó "tenemos manera de saber si se actualizaron los datos de algún candidato ya procesado? por ejemplo se actualizó el cv en TT" — al rastrear el pipeline se descubrió un agujero entre `parse:cvs` y `extract:cvs`: `listPendingExtractions` (`src/lib/cv/extraction/list-pending.ts:51-55`, introducido en Bloque 17) deduplicaba por `(file_id, model, prompt_version)` ignorando `content_hash`. Cuando TT actualiza un CV de un candidato ya extraído, `files` upsert ve hash binario nuevo → row actualizada con `parsed_text=NULL`, `parse:cvs` re-parsea OK, pero `extract:cvs` filtra el file porque la row vieja de `candidate_extractions` (con hash viejo, mismo file_id) sigue ahí — `experiences` y `experience_skills` quedan apuntando a la extracción obsoleta, el matching scorea contra el CV viejo. El bug nunca apareció antes porque el loop habitual del owner es traer candidatos nuevos (row inexistente → no excluida → extract corre); solo se manifiesta en el flujo "TT updates CV", operativamente raro. **Decisión** (ADR-029, commit `d383f37`): sin schema change. El `content_hash` (`SHA256(parsed_text || NUL || model || NUL || prompt_version)`) ya codifica `parsed_text` por construcción — la información existe, solo había que usarla. Dos cambios coordinados: (1) `listPending` lee `content_hash` (no `file_id`), computa por cada file `expectedHash = extractionContentHash(file.parsed_text, model, promptVersion)` e incluye iff `expectedHash NOT IN existingHashes`. Esto alinea el helper con el invariant del schema (`candidate_extractions.content_hash UNIQUE`). (2) Después de cada insertExtraction exitoso, el worker invoca `deps.deleteStaleSiblings({file_id, model, prompt_version, current_hash})` que ejecuta `DELETE WHERE file_id=$1 AND model=$2 AND prompt_version=$3 AND content_hash != $4`. Cascade FKs limpian derivados (`candidate_experiences.extraction_id ON DELETE CASCADE` → `experience_skills.experience_id ON DELETE CASCADE`; `candidate_languages.extraction_id ON DELETE CASCADE`). UPDATE no es opción porque `enforce_raw_output_immutability()` trigger (ADR-012 §4) bloquea mutaciones de `raw_output`. Failure de cleanup loguea a `sync_errors` con `entity='cv_extraction'` pero NO rollbackea — la nueva extracción es válida; la sibling vieja persiste hasta el próximo run (idempotente: el siguiente DELETE encuentra la misma row vieja y la borra). El guard atómico `extractionExistsByHash` (worker line 142) sigue intacto como protección contra race con dos workers paralelos. **TDD estricto**: RED `0ae6cf3` — 3 tests en `list-pending.test.ts` (re_extracts_when_parsed_text_changed, skips_when_text_unchanged_and_hash_matches, existing_query_selects_content_hash_not_just_file_id) + 5 tests en `worker.test.ts` (invokes_after_successful_insert, skips_when_extraction_skipped_by_hash, skips_when_extraction_failed, cleanup_error_is_logged_and_does_not_rollback, skips_when_dep_is_absent_backwards_compat). GREEN `4084520` — implementación + wiring en `extract-cvs.ts` + dos legacy tests en `list-pending.test.ts` actualizados a la nueva semántica (excluded fixtures keyed por `content_hash` en lugar de `file_id`; mock builder chain `.is().not().is()` consistente con el orden real). Suite: **736/736 unit verde, 150/150 integration verde** (incluye los 3 cv/extraction-worker integration tests), typecheck + lint limpios. **Verificación operativa pendiente** (manual): re-correr `pnpm extract:cvs` contra dev DB después de simular `UPDATE files SET parsed_text = parsed_text || E'\n\n[manual change]' WHERE id='<file_id>'`; verificar que el counter `processed > 0` y que la row vieja de `candidate_extractions` desaparece tras el run. **Tech debt restante**: (a) `worker-runtime.loadCandidatesPage` sigue con `.in(candidateIds)` no chunked (Bloque 16 leftover); (b) tooltip "raw years · decay factor" en results table (Bloque 18/19 follow-up); (c) badge visual "decay-aware" en runs post-Bloque-18 vs pre; (d) calibración empírica de `HALF_LIFE_YEARS = 4`; (e) `extract:cvs` log line aún no incluye `languages=` (counter está, solo falta el `console.log`).
**Última sesión previa**: 2026-04-27 — **Bloque 20: tech debt sweep — ADR-027 (cursor persistence) + ADR-028 (sync:full / sync:backfill orchestrators)**. Owner pidió "arregla 1 y 2" del backlog identificado en Bloques 16-17: (1) `sync_state.last_cursor` nunca persistido (cada `runIncremental` arrancaba desde la primera página de TT, "incremental" era misnomer); (2) `sync:full` y `sync:backfill` declarados en `package.json` y referenciados en `.github/workflows/backfill.yml:79-83` pero los archivos `src/scripts/sync-full.ts`/`backfill.ts` no existían — workflow roto para `entity=all` y para backfill puntual. **ADR-027** (`c088c6c`): semántica dual deliberada — `last_synced_at` queda como métrica operativa (cuándo terminó el último run, lo lee el monitoring), `last_cursor` como state del syncer (qué watermark mandar a TT en el próximo run). Se persisten al mismo valor (`runStartedAt`) hoy porque es lo único que el syncer sabe; mañana cuando TT migre a page-token cursors el contrato no cambia. Backward compat: `cursor = acquired.lastCursor ?? acquired.lastSyncedAt` — runs viejos donde `last_cursor=null` siguen funcionando contra el watermark legacy. **TDD estricto** en `src/lib/sync/run.ts`: RED `ecacf22` con 5 tests adversariales (persist on success, prefer cursor over synced_at, fallback when cursor null, both null = first ever run, no advance on error path), GREEN `2cb2ae0` con dos líneas mínimas (`const cursor = acquired.lastCursor ?? acquired.lastSyncedAt;` + `lastCursor: runStartedAt` en releaseLock success). **ADR-028** (`8e8d752`): `sync:full` itera CANONICAL_ENTITY_ORDER fail-fast (stages → users → jobs → custom-fields → candidates → applications → notes → evaluations → files, refleja FK reales) compartiendo SupabaseClient + TeamtailorClient + rate-limit; re-lee `scopeCandidateTtIds` por entidad para que los hijos vean los upserts de candidates del mismo orchestration. `sync:backfill --entity=<X|all>` resetea `last_cursor + last_synced_at` a NULL antes de invocar `runIncremental`, forzando full re-fetch desde page 1 (Operación Tier 2 — gate humano vive en workflow `confirm: yes`). **Helper compartido** extraído a `src/lib/sync/cli.ts` (`requireEnv`, `parseIntEnv`, `parseOptionalPositiveInt`, `parseBoolEnv`, `loadScopeCandidateTtIds`, `resetCursor`, `buildSyncers`, `buildTeamtailorClient`) — `sync-incremental.ts` se refactoriza para consumir el helper (`c9be583`), pasa de ~220 a 116 líneas. Lista canónica de entidades vive en `src/lib/sync/orchestration.ts` con `Object.freeze` y test de paridad con keys de `buildSyncers()` (no más folklore en runbooks). **TDD orchestration**: RED `4dfb0a2` con 12 tests (canonical order frozen + length=9, parseBackfillArgs happy/missing/empty/unknown, runOrchestration empty/single/many/in-order/fail-fast/wraps-error-with-entity-name), GREEN `4cff8ec`. **TDD cli invariants**: 2 tests pin "registry keys = canonical set" y "syncer.entity matches registry key". **Scripts** (`37e7faa`): `sync-full.ts` y `backfill.ts` orquestan vía `runOrchestration` con `runOne` por-entidad que (a) loguea con prefix `[sync:full]`/`[sync:backfill]`, (b) re-lee scope set, (c) llama `runIncremental`. Errores wrappeados por orchestrator se desempaquetan via `e.cause` para mantener el routing de exit codes idéntico a `sync-incremental` (3=lock busy, 2=unknown entity, 4=fatal). **Suite**: 728/728 unit verde (sumó 13 orchestration + 5 ADR-027 + 2 cli invariants), typecheck + lint limpios. **Tech debt restante (post-sweep)**: (a) `worker-runtime.loadCandidatesPage` sigue con `.in(candidateIds)` no chunked (latente hasta caller >100 ids); (b) tooltip "raw years · decay factor" en results table — UX follow-up de Bloque 18/19; (c) badge visual "decay-aware" en runs post-Bloque-18 vs pre — sin migration para no tocar runs históricos; (d) calibración empírica de `HALF_LIFE_YEARS = 4` con outcomes etiquetados.
**Última sesión previa**: 2026-04-27 — **Bloque 19: UI exposure de `last_used` en results table (ADR-026 follow-up)**. Owner pidió mostrar "hace cuánto que no trabaja en esa tecnología" en la tabla de breakdown del matching para que la columna `years` post-decay no se lea como bug ("React 0.4y" sin contexto de cuándo fue el último uso). **Implementación**: nuevo módulo `src/lib/shared/format-time-ago.ts` — helper puro `formatTimeAgo(iso, now)` con cálculo calendario (year/month diff con UTC components, no aritmética de días promedio que rompía "exactly 1 year ago" → "11mo ago" por leap-year drift). Granularidad: `null|''|unparseable → '—'`, `same day | future → 'now'` (clamp), `0 calendar months → '<1mo ago'`, `< 12 → 'Nmo ago'`, `>= 12 → 'Ny ago'` (floor). 11 tests adversariales en `format-time-ago.test.ts` cubren null/empty/unparseable, future clamp, exactly-1-year, 6mo exacto, partial month floor, día <1mo, caso canónico ADR-026 (`2010-12-31` → `'15y ago'`), floor vs round. **TDD estricto**: RED `842b89f` con stub que typechecks pero falla por assertion (escape `TDD_RED=1`), GREEN `6f1a180` con la implementación calendario. **Wire** commit `915f9b0`: `results-table.tsx` agrega columna `LAST USED` entre `YEARS` y `RATIO`, `now = new Date()` hosteado una vez por panel render (single anchor cross-row, no per-cell drift). UI helper, no scoring — `new Date()` es OK acá; el matching sigue determinístico vía `catalogSnapshotAt`. Suite: 708/708 unit verde, typecheck + lint limpios. **Sin schema change, sin cambios al API HTTP** — los datos ya estaban persistidos en `match_runs.results[].breakdown_json` desde Bloque 18, este bloque solo expone. **Tech debt anotado**: la columna `years` ahora muestra `effective_years` post-decay con el mismo nombre que antes mostraba raw years; los reclutadores que usen runs históricos pre-Bloque-18 verán raw, post-Bloque-18 ven effective — sin migration ni badge visual. Tooltip explicativo "raw: X · decay: ×Y" quedó como follow-up.
...
- **Bootstrap Supabase remoto + ADR-009 caso confirmado** ✅ done —
  2026-05-18 — Bloque 24. 60 migraciones aplicadas a
  `tqyvvtmaouhmfsknfvxq` vía `supabase db push --linked --include-all`.
  Primer push falló en migración 002 (`uuid_generate_v4() does not
  exist`) porque la versión de la migración 001 aplicada al remoto el
  2026-04-17 era anterior a ADR-009 (2026-04-18); workaround una-sola-vez
  vía `supabase db query --linked "alter database postgres set
search_path = public, extensions;"`. Tipos regenerados, commit `cd777c6`.
  `.env.production.local` creado (gitignored) con URL + publishable key +
  project ref + Teamtailor + OpenAI; `SUPABASE_SECRET_KEY` queda
  pendiente — el owner debe pegarlo de la dashboard. ADR-009 addendum
  con CLI variant del fix + query de verificación + sección "Caso
  confirmado". Suite 1020/1023 (3 drift preexistente, no relacionado).
  **Bloquea Fase 3 (auth users) y Fase 4 (data load)** hasta tener el
  `sb_secret_…` en el .env.

- **ADR-030 — `HALF_LIFE_YEARS` 4 → 2 (recalibración decay)** ✅ done —
  2026-05-18 — Bloque 23. Commits `413b063` (docs ADR-030),
  `3e7f9fb` (RED recalibrar asserts numéricos), `b0e8f4a` (GREEN
  flip one-line). Resuelve el caso Emiliano (PHP raw≈10y / 7y
  stale): bajo h=4 effective=2.97 → ratio=1.00 vs baseline
  senior=3; bajo h=2 effective=0.88 → ratio=0.29. Resto de ADR-026
  preserved (formula, `asOf` determinístico, axis gate raw, etc.).
  131/131 tests matching verde. **Pendiente manual**: re-correr JD
  del incidente contra dev para confirmar (a) Emiliano cae del
  rank #1, (b) candidatos activos suben relativo, (c) React
  ongoing intacto. Cierra el item (d) del tech debt de Bloque 21.

- **`extract:cvs` log line incluye `languages=`** ✅ done — 2026-05-18 —
  Bloque 22. One-liner en `src/scripts/extract-cvs.ts:274` exponiendo
  el counter `stats.languagesInserted` que ya venía del worker. Cierra
  el item (e) del tech debt de Bloque 21.
...
- **`tests/integration/rag/{semantic-search,hybrid-search}.test.ts`
  falla 3 asserts contra test DB** — detectado 2026-05-18 al cerrar
  Bloque 22. Los tests esperan hits semánticos sobre candidatos
  seedeados pero el ranker devuelve listas vacías (síntoma: embeddings
  no sembrados en supabase-test, o pgvector index sin populate). 1020
  de 1023 unit+integration pasan; las 3 fallas son aisladas a estos
  dos files. **Plan**: investigar si `seed.ts` para test DB genera
  embeddings o si quedó atrás de migraciones del worker; en sesión
  futura. No bloquea Bloque 22 (cambio scoped a `extract-cvs.ts`).
